...
**Ne pas le relancer.** Il écrit dans `content/sujets/` et `content/realisations/`, deux
répertoires **qui n'existent plus** : les fiches ont depuis été fusionnées dans
`content/projects/`, `statuts` étant la seule chose qui distingue un projet livré d'un
sujet disponible. Le relancer recréerait l'ancienne structure en doublon.

`content/` est la source de vérité du site ; les deux dépôts ont divergé.
...
| Fiche | Correction |
|---|---|
| `projects/geexlab-skeletal-animation.md` | `## Voir Email Christophe Muller` supprimé, `## À FAIRE` devenu `## Objectifs`, checklist `- [ ]` convertie en liste, intro ajoutée |
| `projects/spatialisation-sonos-era-300.md` | nom d'étudiant et chemin `suivi/2025-2026/P3/` retirés de `## Réalisation` |
| `projects/watwen-v2.md` | URL GitLab interne remplacée par une mention |
